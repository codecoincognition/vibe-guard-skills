...
- [ ] Path traversal: user-controlled input used in file path construction without sanitization (e.g., `path.join(baseDir, userInput)` without validation)
- [ ] Template injection: user input rendered inside template literals or server-side templates without escaping
...
- [ ] CORS configured as wildcard (`*`) on endpoints that handle authenticated or sensitive operations
